...
<form name="userForm" action="{{ route('usuarios.store') }}" method="POST">
...
<a href="{{ route('usuarios.index') }}" class="btn btn-danger">Cancel</a>
</div>
</div>
</form>
</div>
...
<tr><td></td><td style="text-align: right "><a href="{{ route('usuarios.index') }}" class="btn btn-danger">OK</a> </td></tr>
</table>
</div>
</div>
</div>
...
var user_id = $(this).data('id');
$.get('usuarios/'+user_id, function (data) {
...
$.ajax({
type: "DELETE",
url: "http://localhost/prueba/public/usuarios/"+user_id,
data: {
"id": user_id,
"_token": token,
},
